...
var _ = Describe("HazelcastEndpoint CR", func() {
	...
	When("Hazelcast is updated", func() {
		It("should update the hz endpoint resources if exposeExternally is updated", func() {
			hz := &hazelcastv1alpha1.Hazelcast{
				ObjectMeta: randomObjectMeta(namespace),
				Spec:       test.HazelcastSpec(defaultHazelcastSpecValues()),
			}
		...
		It("should delete the leftover resources if exposeExternally is disabled", func() {
			hz := &hazelcastv1alpha1.Hazelcast{
				ObjectMeta: randomObjectMeta(namespace),
				Spec:       test.HazelcastSpec(defaultHazelcastSpecValues()),
			}
		...
		It("should update new hz endpoint resources if advancedNetwork wan is update", func() {
			hz := &hazelcastv1alpha1.Hazelcast{
				ObjectMeta: randomObjectMeta(namespace),
				Spec:       test.HazelcastSpec(defaultHazelcastSpecValues()),
			}
